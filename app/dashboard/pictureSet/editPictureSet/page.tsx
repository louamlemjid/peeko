...
export default function EditPictureSetsPage() {
  const [sets, setSets] = useState<PictureSet[]>([]);
  const [allAnimations, setAllAnimations] = useState<Animation[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<Partial<PictureSet & { selectedAnimationIds: string[] }>>({});
  ...
  const startEditing = (set: PictureSet) => {
  ...
  const saveEdit = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/pictureSet/${id}`, {
        method: "PUT",
  ...
  const deleteSet = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/v1/pictureSet/delete/${id}`, {
        method: "DELETE",
      });
